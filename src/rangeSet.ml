...
module Make_Discrete(Ord: OrderedType_Discrete): S_Discrete with type elt = Ord.t =
  struct
    ...
    let min_elt s= min (S.min_elt s).start

    let min_elt_opt s= try Some (min_elt s) with Not_found-> None

    let max_elt s= max (S.max_elt s).stop

    let max_elt_opt s= try Some (max_elt s) with Not_found-> None

    let min_elt_of_range r= min r.start
    let max_elt_of_range r= max r.stop
